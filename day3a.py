...
def mm(p, d):
    ...
    for x in p.split(','):
        if x[0] not in d:
            continue
        c += int(x[1:]) * [-1, 1][d.index(x[0])]
        i = min(i, c)
        a = max(a, c)
    return max(abs(i), abs(a))

START_POINT = (
    max(mm(path1, "LR"), mm(path2, "LR")),
    max(mm(path1, "UD"), mm(path2, "UD")),
)
shape = (START_POINT[0]*2 + 1, START_POINT[1]*2 + 1)
...
for x in range(shape[0]):
    for y in range(shape[1]):
        if a3[x,y] and (x, y) != START_POINT:
            d.append(dst((x,y), START_POINT))
